...
- `webServer: { command: string, port?: number, timeout?: number, env?: object }` - Launch a web server before the tests will start.
...
This will automatically launch and wait for the web server to be available before starting the tests:
